...
function Contacts({ leads, moveLead, updateLeadOwner, deleteLead, onAdd }: { leads: Lead[]; moveLead:(id:string,status:string)=>void; updateLeadOwner:(ids:string[],owner:string)=>void; deleteLead:(id:string)=>void; onAdd:()=>void }) {
  const [search, setSearch] = useState(""); const [status, setStatus] = useState("Todos"); const [selected, setSelected] = useState<string[]>([]); const [bulkOwner, setBulkOwner] = useState(responsibleOwners[0]); const [copyMessage, setCopyMessage] = useState("");
  ...
  const selectFirst = (amount:number) => setSelected((ids)=>[...new Set([...ids,...filtered.slice(0,amount).map((l)=>l.id)])]);
