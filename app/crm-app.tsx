...
function Contacts({ leads, workspace, moveLead, updateLead, updateLeadOwner, deleteLead, onAdd, onImport }: { leads: Lead[]; workspace:Workspace; moveLead:(id:string,status:string)=>void; updateLead:(id:string,lead:Partial<Lead>)=>Promise<void>; updateLeadOwner:(ids:string[],owner:string)=>void; deleteLead:(id:string)=>void; onAdd:()=>void; onImport?:()=>void }) {
  const [search, setSearch] = useState(""); const [status, setStatus] = useState("Todos"); const [selected, setSelected] = useState<string[]>([]); const [bulkOwner, setBulkOwner] = useState(responsibleOwners[0]); const [copyMessage, setCopyMessage] = useState(""); const [range, setRange] = useState(""); const [rangeMessage, setRangeMessage] = useState(""); const [editingLead, setEditingLead] = useState<Lead | null>(null);
...
function downloadCsv(leads:Lead[],workspace:Workspace){const obra=workspace==="obra";const headers=obra?["Negocio","Email","Telefono","Rubro","Responsable","Estado","Notas"]:["Negocio","Email","Telefono","Rubro","Responsable","Estado","Prioridad","Tanda","Notas"];const esc=(v:string)=>`"${String(v??"").replace(/"/g,'""')}"`;const rows=leads.map((l)=>obra?[l.businessName,l.email,l.phone,l.segment,l.owner,l.status,l.notes]:[l.businessName,l.email,l.phone,l.segment,l.owner,l.status,l.priority,l.batch,l.notes]);const csv=[headers,...rows].map((r)=>r.map(esc).join(",")).join("\n");const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\ufeff"+csv],{type:"text/csv;charset=utf-8"}));a.download=obra?"prospectos_sincro_obra.csv":"prospectos_sincro.csv";a.click();URL.revokeObjectURL(a.href)}
